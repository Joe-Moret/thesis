...
write.csv(summary_panel_HVZ, file = "results/stats_summary_HVZ.csv", row.names = FALSE)
write.csv(summary_panel_LM, file = "results/stats_summary_LM.csv", row.names = FALSE)

# remove redundant objects
rm(data_temp, summarise_stats, summary_panel_HVZ, summary_panel_LM)



# Panel B: HVZ: regression results -----------------------------------
# 1.Forecast Regression (for up to 5 years into the future) -----------------------------------
# Create a temporary dataset
data_temp <- data %>%
  select(UGVKEY, mapped_fyear, A, D, DD, E, NegE, AC, dependent_E)

# Step 3: Remove NA values for the relevant variables
data_temp <- data_temp %>%
  filter(!is.na(A) & !is.na(D) & !is.na(DD) & !is.na(E) & !is.na(NegE) & !is.na(AC) & !is.na(dependent_E))

# Step 4: Create lagged dependent variables for forecast horizons 1 to 5 years
for (k in 1:5) {
  data_temp <- data_temp %>%
    group_by(UGVKEY) %>% 
...
# Step 5: Function to run rolling regression with Newey-West standard errors for each forecast horizon
...
# Step 6: List to store results for all forecast horizons
all_results <- list()

# Step 7: Run the rolling regression for forecast horizons 1 to 5
for (k in 1:5) {
...
# Step 9: Calculate average R-squared, intercept, coefficients, and t-statistics for each forecast horizon
...
# Step 10: Format the summary results for table output without italics for t-statistics
...
run_rolling_regression <- function(data_temp, forecast_horizon) {
  ...
  for (year in 1968:2023) {
    ...
    data_subset <- winsorize(data_subset, c("A", "D", "E", "AC"))
...
# Winsorize the relevant columns
data_temp <- winsorize(data_temp, cols = c("A", "D", "E", "AC"))
...
# Remove redundant objects
rm(winsorize, data_temp, results, summary_results, run_rolling_regression, summary_table)
...
# 1. Without Division among Market Cap and Book-to-Market: WINSORIZED -----------------------------------
# Define the winsorize function
winsorize <- function(df, cols, lower_limit = 0.005, upper_limit = 0.995) {
  df %>%
    mutate(across(all_of(cols), ~ {
